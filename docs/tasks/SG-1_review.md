...
> | 生产部署（2026-09-22 批准执行） | api.vxquant.com `/mnt/deploy/botflow`，bundle 中转 + `ff-only` → HEAD=`07a151c`、DIRTY=0；三重备份落盘、探活 91s/连续 31×200、`call_attempts` 迁移成功；**回滚点 `eb81d1d`** + DB 快照 `pre-sg1-db-20260922-135708.db`（详见 §4.2） |
> | 外部回归（生产复跑） | **E1–E10 = 23/23 PASS**（与 mq3 逐项一致）；AD-1 两条 NOTE 同样按预期复现 |
>
...
### 4.2 生产部署与复跑（2026-09-22，用户批准执行）

生产从 `eb81d1d`（v3.0.0，早于 SG-0）一次跨 SG-0 + SG-1 升级到 `07a151c`（v3.1.0），共 21 个提交。

**部署前体检发现并解决的 4 个阻断项**：

| # | 阻断项 | 解法 |
|---|---|---|
| 1 | 生产到 GitHub 的 SSH/HTTPS 均不通（`publickey` / `GnuTLS -110`），无法 `git fetch` | 本机 `git bundle create eb81d1d..main` → scp 中转 → 生产 `git fetch /tmp/botflow_deploy.bundle`（MD5 双端一致） |
| 2 | `tiktoken` 未安装（增量 `6ce7f87` 的硬依赖） | `uv sync --no-dev` → `tiktoken==0.14.0` + `regex==2026.9.10`、`langsmith 0.12.4→0.14.0` |
| 3 | `supervisorctl` 无权限（需 sudo） | 生产 `sudo -n` 免密可用 |
| 4 | 生产库无 `call_attempts` 表（SG-0 将新建），库 324 MB | 升级前 sqlite online backup API 落 DB 快照（339,935,232 B / 2.5s）+ 代码 tgz + git 回滚点三重备份 |

**执行与验收**（脚本 `_deploy.sh`，任一步失败即中止、不带病重启）：

1. 三重备份 → bundle fetch → `merge-base` 确认可 FF → `merge --ff-only` → DIRTY=0；
2. `uv sync --no-dev` → `botflow --version` = **3.1.0**；
3. tiktoken 词表预热（**78.8s 首次下载**，缓存 `/home/openbot/.cache/tiktoken`）；supervisor conf 追加
   `TIKTOKEN_CACHE_DIR`（防重启后 `/tmp` 缓存丢失导致请求期在线下载），`reread` 生效；
4. `sudo -n supervisorctl restart botflow` → 探活 **91s / 连续 31 次 200**（新 pid 409676）；
5. 迁移校验：`call_attempts` exists、`call_logs`=64,916 行无损；`/health` 正常；
6. **生产复跑 E1–E10 = 23/23 PASS**，与 mq3 逐项一致（E9-1 2 跳成功 / E9-2 上限 3 组 / E9-3 环检测 /
   E10 502 + `ConfigurationError` id=64933）；E9/E10 临时分组前缀 `prd_e*` **零残留**；
7. 部署后错误扫描：仅 5 条 error，全部为探针预期产物（2×AD-1 AttributeError、2×NoAvailableModelError、
   1×ConfigurationError），真实流量 status=`success` 全绿。

**观测记录**：`reread` 同时报出 `nanobot-gateway: changed`（与本次改动无关的**既有**待应用 conf 变更，
本次未动它、也未执行 `update`，该程序仍以旧 conf 运行）。`call_logs.status` 的成功值是 **`success` 不是 `ok`**。
...
5. **生产已部署 SG-1（2026-09-22 执行）**：生产已从 `eb81d1d` 升级到 `07a151c` 并完成
   `SG-1_tests.md` §4 要求的「上生产后再跑一遍」（E1–E10 = 23/23 PASS，§4.2）。
   回滚预案：git 回滚点 `eb81d1d` + 代码 tgz + DB 快照三重备份均在 `/mnt/deploy/backups/`。
...
| **生产部署 SG-1** | ~~暂不部署，待单独批准~~ → **已批准并执行（2026-09-22）** | 生产 `eb81d1d` → `07a151c`（v3.1.0，一次跨 SG-0 + SG-1，含「降级 1→3 组」「未知 type 改 502」两条行为变更）。部署流程、备份与生产复跑证据见 **§4.2**；回滚点 `eb81d1d` + 三重备份在 `/mnt/deploy/backups/` |

> 副作用提示：因 AD-1 暂不修，**AD-1 现随本次部署上线并在生产按原样复现**（§4.2 复跑 E6/E8 已确认，
> `call_logs` 两条 AttributeError 即探针所留）；由于历史真实触发为 0 次，风险窗口仅在
> 「有客户端首次使用 anthropic / responses 流式端点」时打开——建议在客户端接入这两个端点**之前**排期修复。
...
- 处置决策：**AD-1 只记录暂不修**（§7.1）、**生产部署已批准并执行**（`eb81d1d` → `07a151c`，生产复跑
  E1–E10 = 23/23 PASS，§4.2）；两者均不影响 SG-1 的验收结论。
